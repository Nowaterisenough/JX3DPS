...
struct alignas(64) TickCache {
     ...
     */
    tick_t GetMinTick(tick_t gcd) const {
        tick_t min_tick = gcd > 0 ? gcd : std::numeric_limits<tick_t>::max();
...
struct MacroTimeThreshold {
    ...
    MacroTimeThreshold(Type t, size_t idx, tick_t thr)
        : type(t), cache_index(idx), threshold(thr) {}
};
...
struct SimContext {
    ...
    // 宏时间阈值列表（用于生成精确关键帧）
    std::vector<MacroTimeThreshold> macro_time_thresholds;
     ...
     */
    tick_t GetNextKeyFrameTick() const {
        // 先从 TickCache 获取基础的最小 tick
        tick_t min_tick = cache.GetMinTick(globalCooldownCurrent);

        // 检查宏时间阈值
        for (const auto &threshold : macro_time_thresholds) {
            tick_t current_value = 0;

            switch (threshold.type) {
                case MacroTimeThreshold::Type::SKILL_COOLDOWN:
                    current_value = cache.skill_cooldown[threshold.cache_index];
                    break;
                case MacroTimeThreshold::Type::BUFF_DURATION:
                    current_value = cache.buff_duration[0][threshold.cache_index];
                    break;
                case MacroTimeThreshold::Type::TBUFF_DURATION:
                    // 目标BUFF，假设第一个目标
                    if (!targets.empty()) {
    ...
    void Reset() {
        ...
        std::memset(skill_cooldown_list, 0, sizeof(skill_cooldown_list));
    }
};

// 线程本地上下文
extern thread_local SimContext tls_sim;
...
void BindCooldownImpl() noexcept {
    if constexpr (requires { Skill::cooldown; Skill::current_cooldown; }) {
        static int cd_index                   = 0;
        Skill::current_cooldown               = &tls_sim.cache.skill_cooldown[tls_sim.bind_cooldown_index];
        tls_sim.skill_cooldown_list[cd_index] = tls_sim.bind_cooldown_index;
        ++cd_index;
    }
}
...
void BindCooldown(tick_t cd) noexcept {
    ...
    tls_sim.skill_cooldown_list[tls_sim.bind_cooldown_index] = cd;
    ++tls_sim.bind_cooldown_index;
}
...
struct Target {
    ...
    bool HasBuff(jx3id_t buffId) const { return buffs.find(buffId) != buffs.end(); }

    int GetBuffStack(jx3id_t buffId) const {
        auto it = buffs.find(buffId);
    ...
    void ClearBuff(jx3id_t buffId) { buffs.erase(buffId); }
