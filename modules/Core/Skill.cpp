...
Skill::Skill(Player *player) :
    m_player(player), m_publicCooldown(&m_player->m_publicCooldown)
{
...
{
    TableRes tableRes = Roll(m_player->Attr().GetPhysicsHitValuePercent() + m_skillHitValuePercentAdd,
...
DamageStats Skill::GetPhysicsDamage(Target &target, TableRes tableRes, std::string &subName, int level)
{
    ...
    return DamageStats(FINAL_DAMAGE(fixedDamage,
                                    tableResNum,
                                    (m_player->Attr().GetPhysicsCriticalStrikePowerPercent() + m_skillCriticalStrikePowerPercentAdd)),
...
void Skill::UpdatePhysicsStats(Target &target, Stats &stats, SIM_MODE mode, TableRes tableRes, std::string &subName, int level)
{
...
{
    Pct_t damagePercent = ((MAX_BIN_PCT_CONST + m_skillDamageBinPercentAdd) / static_cast<double>(MAX_BIN_PCT_CONST)) *
                          (1 + m_player->Attr().GetMagicOvercomePercent()) *
                          (1 + target.GetDamagePercentAdd()) *
    ...
    return DamageStats(FINAL_DAMAGE(fixedDamage,
                                    tableResNum,
                                    (m_player->Attr().GetMagicCriticalStrikePowerPercent() + m_skillCriticalStrikePowerPercentAdd)),
                       FINAL_DAMAGE(weaponDamage,
                                    tableResNum,
                                    (m_player->Attr().GetMagicCriticalStrikePowerPercent() + m_skillCriticalStrikePowerPercentAdd)),
                       FINAL_DAMAGE(attackDamage,
...
void Skill::UpdateMagicStats(Target &target, Stats &stats, SIM_MODE mode, TableRes tableRes, std::string &subName, int level)
{
    DamageStats damageStats;
        switch (static_cast<int>(mode)) {
        case SIM_MODE::DEFAULT :
