...
Target::Target(Player &player, const Id_t id, const int level, const Value_t shield, const Class classType) :
    m_id(id),
    m_level(level),
    m_lifePercent(1.0),
    m_shieldIgnoreBinPercent(0),
    m_physicsShield(0),
    m_physicsShieldBinPercent(0),
    m_physicsResistPercent(0.0),
    m_magicShield(0),
    m_magicShieldBinPercent(0),
    m_magicResistPercent(0.0),
    m_damageAddBinPercent(0),
...
void Target::UpdatePhysicsResistPercent()
{
    ...
    m_physicsResistPercent = physicsShield / (physicsShield + PHYSICS_SHIELD_PARAM * (LEVEL_PARAM * m_level - LEVEL_CONST));
...
void Target::UpdateMagicResistPercent()
{
    Value_t magicShield = static_cast<int>(m_magicShield *
                                           ...
                                           (static_cast<Pct_t>(MAX_BIN_PCT_CONST - m_shieldIgnoreBinPercent) / MAX_BIN_PCT_CONST));
    m_magicResistPercent = magicShield / (magicShield + MAGIC_SHIELD_PARAM * (LEVEL_PARAM * m_level - LEVEL_CONST));
