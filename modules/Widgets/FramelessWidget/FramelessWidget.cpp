/**
 * Project: JX3DPS
 * File: FramelessWidget.cpp
 * Description:
 * Created Date: 2023-06-16 08:21:30
 * Author: 难为水
 * -----
 * Last Modified: 2023-06-18 00:50:57
 * Modified By: 难为水
 * -----
 * HISTORY:
 * Date      	By   	Comments
...
{
    if (widget && event->type() == QEvent::MouseButtonRelease) {
        // 恢复所有
    ...
    if (widget && watched == widget) {
        if (event->type() == QEvent::WindowStateChange) {
            ...
            if (resizeEnable) {
                ...
                } else if (pressedRect.at(1).contains(point) && widget->maximumWidth() != widget->minimumWidth())
                {
                    widget->setCursor(Qt::SizeHorCursor);
                } else if (pressedRect.at(2).contains(point) && widget->maximumHeight() != widget->minimumHeight())
                {
                    widget->setCursor(Qt::SizeVerCursor);
                } else if (pressedRect.at(3).contains(point) && widget->maximumHeight() != widget->minimumHeight())
                {
                    widget->setCursor(Qt::SizeVerCursor);
...
void FramelessWidget::SetMousePressed(bool mousePressed)
{
...
void FramelessWidget::SetWidget(QWidget *widget)
{
    if (this->widget == 0) {
        ...
        this->widget->setAttribute(Qt::WA_NoMousePropagation, true);
