    ...
    for (const auto &str : macro) {
      ...
      for (const auto &[i, n] : ids) {
        ...
        temp = std::regex_replace(
            temp, reg,
            "宏·" + std::to_string(i - EXPRESSION_SKILL_PLACE_HOLDERS_DEFAULT));
        if (name == n) {
          ii = i;
        }
      }
...
JX3DPS::Error_t JX3DPS::Regex::ParseToExprSkill(const std::string &str,
                                                ExprSkill &exprSkill) {
  std::smatch mat;
  ...
  if (cast == JX3DPS_REGEX_EXPRESSION_SKILL_TYPE_SCAST ||
      cast == JX3DPS_REGEX_EXPRESSION_SKILL_TYPE_SFCAST) {
    scast.emplace_back(std::bind(&JX3DPS::Expression::IsScast,
  ...
  std::regex regChangeExpression(
      JX3DPS_REGEX_EXPRESSION_SKILL_CHANGE_EXPRESSION);
  if (std::regex_match(str, mat, regChangeTarget)) {
    Id_t id = static_cast<Id_t>(std::stoi(mat[3].str()) +
                                TARGET_PLACE_HOLDERS_DEFAULT);
    exprSkill.second = id;
    ...
    Id_t id = static_cast<Id_t>(std::stoi(mat[3].str()) +
                                EXPRESSION_SKILL_PLACE_HOLDERS_DEFAULT);
    exprSkill.second = id;
    if (cast == JX3DPS_REGEX_EXPRESSION_SKILL_TYPE_FCAST ||
        cast == JX3DPS_REGEX_EXPRESSION_SKILL_TYPE_SFCAST) {
      exprSkill.first.front().emplace_back(
          std::bind(&JX3DPS::Expression::StopReCastSkill, std::placeholders::_1,
                    std::placeholders::_2));
    }
  } else {
    ...
    exprSkill.second = id;
    // 添加冷却判断
    pre.emplace_back(std::bind(&JX3DPS::Expression::IsCooldown,
                               std::placeholders::_1, std::placeholders::_2,
                               id));
    // 添加是否在范围内判断
    ...
    pre.emplace_back(std::bind(&JX3DPS::Expression::IsReady,
  ...
  while (condition.find(JX3DPS_REGEX_EXPRESSION_SKILL_SYMBOL_OR) !=
         std::string::npos) {
    temp = condition.substr(
        0, condition.find(JX3DPS_REGEX_EXPRESSION_SKILL_SYMBOL_OR));
    std::list<ExprIf> exprIfs;
    while (temp.find(JX3DPS_REGEX_EXPRESSION_SKILL_SYMBOL_AND) !=
           std::string::npos) {
      temp2 =
          temp.substr(0, temp.find(JX3DPS_REGEX_EXPRESSION_SKILL_SYMBOL_AND));
      ...
      temp =
          temp.substr(temp.find(JX3DPS_REGEX_EXPRESSION_SKILL_SYMBOL_AND) + 1);
    }
    ...
    condition = condition.substr(
        condition.find(JX3DPS_REGEX_EXPRESSION_SKILL_SYMBOL_OR) + 1);
  }

  while (condition.find(JX3DPS_REGEX_EXPRESSION_SKILL_SYMBOL_AND) !=
         std::string::npos) {
    temp2 = condition.substr(
        0, condition.find(JX3DPS_REGEX_EXPRESSION_SKILL_SYMBOL_AND));
    err = ParseToExprIf(temp2, exprIf);
    if (err != JX3DPS_SUCCESS) {
      return err;
    }
    exprIfs.emplace_back(exprIf);
    condition = condition.substr(
        condition.find(JX3DPS_REGEX_EXPRESSION_SKILL_SYMBOL_AND) + 1);
  }
...
const char *const JX3DPS_REGEX_EXPRESSION_EVENT_TYPE_CHANGE_TARGET =
    "change_target";
...
JX3DPS::Error_t
JX3DPS::Regex::ParseToExprEvents(const std::list<std::string> &strs,
  ...
  for (const auto &str : strs) {
    ...
    if (mat[2].str() == JX3DPS_REGEX_EXPRESSION_EVENT_TYPE_ADD_TARGET) {
      ...
      exprEvents.insert(exprEvents.end(), exprs.begin(), exprs.end());
               ...
               JX3DPS_REGEX_EXPRESSION_EVENT_TYPE_CHANGE_TARGET) {
...
const char
    *const JX3DPS_REGEX_EXPRESSION_SKILL_CONDITION_TARGET_BUFF_STACK_NUM =
...
JX3DPS::Error_t JX3DPS::Regex::ParseToExprIf(const std::string &str,
  ...
  std::regex regBuffStackNum(
      JX3DPS_REGEX_EXPRESSION_SKILL_CONDITION_BUFF_STACK_NUM);
  ...
  std::regex regTbuffStackNum(
      JX3DPS_REGEX_EXPRESSION_SKILL_CONDITION_TARGET_BUFF_STACK_NUM);
  std::regex regTbuffTime(
      JX3DPS_REGEX_EXPRESSION_SKILL_CONDITION_TARGET_BUFF_TIME);
  std::regex regLastCastSkill(
      JX3DPS_REGEX_EXPRESSION_SKILL_CONDITION_LAST_SKILL);
  std::regex regSkillCd(JX3DPS_REGEX_EXPRESSION_SKILL_CONDITION_SKILL_COOLDOWN);
  std::regex regSkillNotinCd(
      JX3DPS_REGEX_EXPRESSION_SKILL_CONDITION_SKILL_NOTIN_COOLDOWN);
  std::regex regSkillEnergy(
      JX3DPS_REGEX_EXPRESSION_SKILL_CONDITION_SKILL_ENERGY);

  std::smatch mat;
  if (std::regex_match(str, mat, regQidian)) {
    if (mat[1].str() == "<") {
      exprIf = std::bind(&JX3DPS::Expression::QidianLt, std::placeholders::_1,
                         std::placeholders::_2, std::stoi(mat[2].str()));
    } else if (mat[1].str() == "<=") {
      exprIf = std::bind(&JX3DPS::Expression::QidianLe, std::placeholders::_1,
                         std::placeholders::_2, std::stoi(mat[2].str()));
    } else if (mat[1].str() == "=") {
      exprIf = std::bind(&JX3DPS::Expression::QidianEq, std::placeholders::_1,
                         std::placeholders::_2, std::stoi(mat[2].str()));
    } else if (mat[1].str() == "~=") {
      exprIf = std::bind(&JX3DPS::Expression::QidianNe, std::placeholders::_1,
                         std::placeholders::_2, std::stoi(mat[2].str()));
    } else if (mat[1].str() == ">=") {
      exprIf = std::bind(&JX3DPS::Expression::QidianGe, std::placeholders::_1,
                         std::placeholders::_2, std::stoi(mat[2].str()));
    } else if (mat[1].str() == ">") {
      exprIf = std::bind(&JX3DPS::Expression::QidianGt, std::placeholders::_1,
                         std::placeholders::_2, std::stoi(mat[2].str()));
    }
  } else if (std::regex_match(str, mat, regTLife)) {
    JX3DPS::Id_t id = TargetId(mat[1].str());
    if (mat[2].str() == "<") {
      exprIf = std::bind(&JX3DPS::Expression::TLifeLt, std::placeholders::_1,
                         std::placeholders::_2, id, std::stod(mat[3].str()));
    } else if (mat[2].str() == "<=") {
      exprIf = std::bind(&JX3DPS::Expression::TLifeLe, std::placeholders::_1,
                         std::placeholders::_2, id, std::stoi(mat[3].str()));
    } else if (mat[2].str() == "=") {
      exprIf = std::bind(&JX3DPS::Expression::TLifeEq, std::placeholders::_1,
                         std::placeholders::_2, id, std::stod(mat[3].str()));
    } else if (mat[2].str() == "~=") {
      exprIf = std::bind(&JX3DPS::Expression::TLifeNe, std::placeholders::_1,
                         std::placeholders::_2, id, std::stod(mat[3].str()));
    } else if (mat[2].str() == ">=") {
      exprIf = std::bind(&JX3DPS::Expression::TLifeGe, std::placeholders::_1,
                         std::placeholders::_2, id, std::stod(mat[3].str()));
    } else if (mat[2].str() == ">") {
      exprIf = std::bind(&JX3DPS::Expression::TLifeGt, std::placeholders::_1,
                         std::placeholders::_2, id, std::stod(mat[3].str()));
    }
  } else if (std::regex_match(str, mat, regDistance)) {
    JX3DPS::Id_t id = TargetId(mat[1].str());
    if (mat[2].str() == "<") {
      exprIf = std::bind(&JX3DPS::Expression::DistanceLt, std::placeholders::_1,
                         std::placeholders::_2, id, std::stod(mat[3].str()));
    } else if (mat[2].str() == "<=") {
      exprIf = std::bind(&JX3DPS::Expression::DistanceLe, std::placeholders::_1,
                         std::placeholders::_2, id, std::stod(mat[3].str()));
    } else if (mat[2].str() == "=") {
      exprIf = std::bind(&JX3DPS::Expression::DistanceEq, std::placeholders::_1,
                         std::placeholders::_2, id, std::stod(mat[3].str()));
    } else if (mat[2].str() == "~=") {
      exprIf = std::bind(&JX3DPS::Expression::DistanceNe, std::placeholders::_1,
                         std::placeholders::_2, id, std::stod(mat[3].str()));
    } else if (mat[2].str() == ">=") {
      exprIf = std::bind(&JX3DPS::Expression::DistanceGe, std::placeholders::_1,
                         std::placeholders::_2, id, std::stod(mat[3].str()));
    } else if (mat[2].str() == ">") {
      exprIf = std::bind(&JX3DPS::Expression::DistanceGt, std::placeholders::_1,
                         std::placeholders::_2, id, std::stod(mat[3].str()));
    }
  } else if (std::regex_match(str, mat, regBuff)) {
    JX3DPS::Id_t id = BuffId(mat[1].str());
    exprIf = std::bind(&JX3DPS::Expression::BuffExist, std::placeholders::_1,
                       std::placeholders::_2, id);
  } else if (std::regex_match(str, mat, regNoBuff)) {
    JX3DPS::Id_t id = BuffId(mat[1].str());
    exprIf = std::bind(&JX3DPS::Expression::NoBuffExist, std::placeholders::_1,
                       std::placeholders::_2, id);
  } else if (std::regex_match(str, mat, regBuffStackNum)) {
    JX3DPS::Id_t id = BuffId(mat[1].str());
    if (mat[2].str() == "<") {
      exprIf =
          std::bind(&JX3DPS::Expression::BuffStackNumLt, std::placeholders::_1,
                    std::placeholders::_2, id, std::stoi(mat[3].str()));
    } else if (mat[2].str() == "<=") {
      exprIf =
          std::bind(&JX3DPS::Expression::BuffStackNumLe, std::placeholders::_1,
                    std::placeholders::_2, id, std::stoi(mat[3].str()));
    } else if (mat[2].str() == "=") {
      exprIf =
          std::bind(&JX3DPS::Expression::BuffStackNumEq, std::placeholders::_1,
                    std::placeholders::_2, id, std::stoi(mat[3].str()));
    } else if (mat[2].str() == "~=") {
      exprIf =
          std::bind(&JX3DPS::Expression::BuffStackNumNe, std::placeholders::_1,
                    std::placeholders::_2, id, std::stoi(mat[3].str()));
    } else if (mat[2].str() == ">=") {
      exprIf =
          std::bind(&JX3DPS::Expression::BuffStackNumGe, std::placeholders::_1,
                    std::placeholders::_2, id, std::stoi(mat[3].str()));
    } else if (mat[2].str() == ">") {
      exprIf =
          std::bind(&JX3DPS::Expression::BuffStackNumGt, std::placeholders::_1,
                    std::placeholders::_2, id, std::stoi(mat[3].str()));
    }
    ...
    if (mat[2].str() == "<") {
      exprIf =
          std::bind(&JX3DPS::Expression::BuffTimeLt, std::placeholders::_1,
                    std::placeholders::_2, id,
                    std::stod(mat[3].str()) * JX3DPS::JX3_FRAMES_PER_SECOND);
    } else if (mat[2].str() == "<=") {
      exprIf =
          std::bind(&JX3DPS::Expression::BuffTimeLe, std::placeholders::_1,
                    std::placeholders::_2, id,
                    std::stod(mat[3].str()) * JX3DPS::JX3_FRAMES_PER_SECOND);
    } else if (mat[2].str() == "=") {
      exprIf =
          std::bind(&JX3DPS::Expression::BuffTimeEq, std::placeholders::_1,
                    std::placeholders::_2, id,
                    std::stod(mat[3].str()) * JX3DPS::JX3_FRAMES_PER_SECOND);
    } else if (mat[2].str() == "~=") {
      exprIf =
          std::bind(&JX3DPS::Expression::BuffTimeNe, std::placeholders::_1,
                    std::placeholders::_2, id,
                    std::stod(mat[3].str()) * JX3DPS::JX3_FRAMES_PER_SECOND);
    } else if (mat[2].str() == ">=") {
      exprIf =
          std::bind(&JX3DPS::Expression::BuffTimeGe, std::placeholders::_1,
                    std::placeholders::_2, id,
                    std::stod(mat[3].str()) * JX3DPS::JX3_FRAMES_PER_SECOND);
    } else if (mat[2].str() == ">") {
      exprIf =
          std::bind(&JX3DPS::Expression::BuffTimeGt, std::placeholders::_1,
                    std::placeholders::_2, id,
                    std::stod(mat[3].str()) * JX3DPS::JX3_FRAMES_PER_SECOND);
    }
  } else if (std::regex_match(str, mat, regTbuff)) {
    JX3DPS::Id_t id = BuffId(mat[1].str());
    exprIf = std::bind(&JX3DPS::Expression::TBuffExist, std::placeholders::_1,
                       std::placeholders::_2, id);
  } else if (std::regex_match(str, mat, regTNoBuff)) {
    JX3DPS::Id_t id = BuffId(mat[1].str());
    exprIf = std::bind(&JX3DPS::Expression::TNoBuffExist, std::placeholders::_1,
                       std::placeholders::_2, id);
  } else if (std::regex_match(str, mat, regTbuffStackNum)) {
    JX3DPS::Id_t id = BuffId(mat[1].str());
    if (mat[2].str() == "<") {
      exprIf =
          std::bind(&JX3DPS::Expression::TBuffStackNumLt, std::placeholders::_1,
                    std::placeholders::_2, id, std::stoi(mat[3].str()));
    } else if (mat[2].str() == "<=") {
      exprIf =
          std::bind(&JX3DPS::Expression::TBuffStackNumLe, std::placeholders::_1,
                    std::placeholders::_2, id, std::stoi(mat[3].str()));
    } else if (mat[2].str() == "=") {
      exprIf =
          std::bind(&JX3DPS::Expression::TBuffStackNumEq, std::placeholders::_1,
                    std::placeholders::_2, id, std::stoi(mat[3].str()));
    } else if (mat[2].str() == "~=") {
      exprIf =
          std::bind(&JX3DPS::Expression::TBuffStackNumNe, std::placeholders::_1,
                    std::placeholders::_2, id, std::stoi(mat[3].str()));
    } else if (mat[2].str() == ">=") {
      exprIf =
          std::bind(&JX3DPS::Expression::TBuffStackNumGe, std::placeholders::_1,
                    std::placeholders::_2, id, std::stoi(mat[3].str()));
    } else if (mat[2].str() == ">") {
      exprIf =
          std::bind(&JX3DPS::Expression::TBuffStackNumGt, std::placeholders::_1,
                    std::placeholders::_2, id, std::stoi(mat[3].str()));
    }
    ...
    if (mat[2].str() == "<") {
      exprIf =
          std::bind(&JX3DPS::Expression::TBuffTimeLt, std::placeholders::_1,
                    std::placeholders::_2, id,
                    std::stod(mat[3].str()) * JX3DPS::JX3_FRAMES_PER_SECOND);
    } else if (mat[2].str() == "<=") {
      exprIf =
          std::bind(&JX3DPS::Expression::TBuffTimeLe, std::placeholders::_1,
                    std::placeholders::_2, id,
                    std::stod(mat[3].str()) * JX3DPS::JX3_FRAMES_PER_SECOND);
    } else if (mat[2].str() == "=") {
      exprIf =
          std::bind(&JX3DPS::Expression::TBuffTimeEq, std::placeholders::_1,
                    std::placeholders::_2, id,
                    std::stod(mat[3].str()) * JX3DPS::JX3_FRAMES_PER_SECOND);
    } else if (mat[2].str() == "~=") {
      exprIf =
          std::bind(&JX3DPS::Expression::TBuffTimeNe, std::placeholders::_1,
                    std::placeholders::_2, id,
                    std::stod(mat[3].str()) * JX3DPS::JX3_FRAMES_PER_SECOND);
    } else if (mat[2].str() == ">=") {
      exprIf =
          std::bind(&JX3DPS::Expression::TBuffTimeGe, std::placeholders::_1,
                    std::placeholders::_2, id,
                    std::stod(mat[3].str()) * JX3DPS::JX3_FRAMES_PER_SECOND);
    } else if (mat[2].str() == ">") {
      exprIf =
          std::bind(&JX3DPS::Expression::TBuffTimeGt, std::placeholders::_1,
                    std::placeholders::_2, id,
                    std::stod(mat[3].str()) * JX3DPS::JX3_FRAMES_PER_SECOND);
    }
    ...
    if (mat[1].str() == "=") {
      exprIf = std::bind(&JX3DPS::Expression::LastCastSkill,
                         std::placeholders::_1, std::placeholders::_2, id);
    } else if (mat[1].str() == "~=") {
      exprIf = std::bind(&JX3DPS::Expression::NotLastCastSkill,
                         std::placeholders::_1, std::placeholders::_2, id);
    }
  } else if (std::regex_match(str, mat, regSkillNotinCd)) {
    JX3DPS::Id_t id = SkillId(mat[1].str());
    exprIf = std::bind(&JX3DPS::Expression::SkillNotinCd, std::placeholders::_1,
                       std::placeholders::_2, id);
  } else if (std::regex_match(str, mat, regSkillCd)) {
    JX3DPS::Id_t id = SkillId(mat[1].str());
    if (mat[2].str() == "<") {
      exprIf =
          std::bind(&JX3DPS::Expression::SkillCooldownLt, std::placeholders::_1,
                    std::placeholders::_2, id,
                    std::stod(mat[3].str()) * JX3DPS::JX3_FRAMES_PER_SECOND);
    } else if (mat[2].str() == "<=") {
      exprIf =
          std::bind(&JX3DPS::Expression::SkillCooldownLe, std::placeholders::_1,
                    std::placeholders::_2, id,
                    std::stod(mat[3].str()) * JX3DPS::JX3_FRAMES_PER_SECOND);
    } else if (mat[2].str() == "=") {
      exprIf =
          std::bind(&JX3DPS::Expression::SkillCooldownEq, std::placeholders::_1,
                    std::placeholders::_2, id,
                    std::stod(mat[3].str()) * JX3DPS::JX3_FRAMES_PER_SECOND);
    } else if (mat[2].str() == "~=") {
      exprIf =
          std::bind(&JX3DPS::Expression::SkillCooldownNe, std::placeholders::_1,
                    std::placeholders::_2, id,
                    std::stod(mat[3].str()) * JX3DPS::JX3_FRAMES_PER_SECOND);
    } else if (mat[2].str() == ">=") {
      exprIf =
          std::bind(&JX3DPS::Expression::SkillCooldownGe, std::placeholders::_1,
                    std::placeholders::_2, id,
                    std::stod(mat[3].str()) * JX3DPS::JX3_FRAMES_PER_SECOND);
    } else if (mat[2].str() == ">") {
      exprIf =
          std::bind(&JX3DPS::Expression::SkillCooldownGt, std::placeholders::_1,
                    std::placeholders::_2, id,
                    std::stod(mat[3].str()) * JX3DPS::JX3_FRAMES_PER_SECOND);
    }
    ...
    if (mat[2].str() == "<") {
      exprIf =
          std::bind(&JX3DPS::Expression::SkillEnergyLt, std::placeholders::_1,
                    std::placeholders::_2, id, std::stod(mat[3].str()));
    } else if (mat[2].str() == "<=") {
      exprIf =
          std::bind(&JX3DPS::Expression::SkillEnergyLe, std::placeholders::_1,
                    std::placeholders::_2, id, std::stod(mat[3].str()));
    } else if (mat[2].str() == "=") {
      exprIf =
          std::bind(&JX3DPS::Expression::SkillEnergyEq, std::placeholders::_1,
                    std::placeholders::_2, id, std::stod(mat[3].str()));
    } else if (mat[2].str() == "~=") {
      exprIf =
          std::bind(&JX3DPS::Expression::SkillEnergyNe, std::placeholders::_1,
                    std::placeholders::_2, id, std::stod(mat[3].str()));
    } else if (mat[2].str() == ">=") {
      exprIf =
          std::bind(&JX3DPS::Expression::SkillEnergyGe, std::placeholders::_1,
                    std::placeholders::_2, id, std::stod(mat[3].str()));
    } else if (mat[2].str() == ">") {
      exprIf =
          std::bind(&JX3DPS::Expression::SkillEnergyGt, std::placeholders::_1,
                    std::placeholders::_2, id, std::stod(mat[3].str()));
    }
  } else {
    return JX3DPS_ERROR_INVALID_EXPRESSION;
...
JX3DPS::Error_t JX3DPS::Regex::ParseToFrame(const std::string &str,
                                            Frame_t &frame) {
  ...
  frame = (std::stoi(mat[1].str()) * JX3_SECONDS_PER_MINUTE +
           std::stoi(mat[2].str())) *
...
JX3DPS::Error_t JX3DPS::Regex::ParseToAddTarget(const std::string &str,
                                                ExprEvents &exprEvents) {
  ...
  Id_t id =
      static_cast<Id_t>(std::stoi(mat[1].str()) + TARGET_PLACE_HOLDERS_DEFAULT);
  ...
  exprEvents.front().second =
      std::bind(&JX3DPS::Expression::AddTarget, std::placeholders::_1,
                std::placeholders::_2, id, std::stoi(mat[2].str()),
                std::stoi(mat[3].str()), std::stod(mat[4].str()), frame);

  ExprEvent exprEvent;
  exprEvent.first = exprEvents.front().first + frame;
  exprEvent.second =
      std::bind(&JX3DPS::Expression::SetTargetDead, std::placeholders::_1,
                std::placeholders::_2, id);
  exprEvents.emplace_back(exprEvent);
...
JX3DPS::Error_t JX3DPS::Regex::ParseToChangeTarget(const std::string &str,
                                                   ExprEvent &exprEvent) {
  ...
  exprEvent.second =
      std::bind(&JX3DPS::Expression::ChangeTarget, std::placeholders::_1,
                std::placeholders::_2, id);
...
const char *const JX3DPS_REGEX_EXPRESSION_EVENT_TYPE_SET_TARGET_DISTANCE =
    "distance";

const char *const JX3DPS_REGEX_EXPRESSION_EVENT_TYPE_SET_TARGET_DEAD = "dead";

JX3DPS::Error_t JX3DPS::Regex::ParseToSetTarget(const std::string &str,
                                                ExprEvent &exprEvent) {
  ...
  if (mat[2].str() == JX3DPS_REGEX_EXPRESSION_EVENT_TYPE_SET_TARGET_LIFE) {
    exprEvent.second =
        std::bind(&JX3DPS::Expression::SetTargetLife, std::placeholders::_1,
                  std::placeholders::_2, id, std::stod(mat[4].str()));
  } else if (mat[2].str() ==
             JX3DPS_REGEX_EXPRESSION_EVENT_TYPE_SET_TARGET_MANA) {
    exprEvent.second =
        std::bind(&JX3DPS::Expression::SetTargetMana, std::placeholders::_1,
                  std::placeholders::_2, id, std::stod(mat[4].str()));
  } else if (mat[2].str() ==
             JX3DPS_REGEX_EXPRESSION_EVENT_TYPE_SET_TARGET_DISTANCE) {
    exprEvent.second =
        std::bind(&JX3DPS::Expression::SetTargetDistance, std::placeholders::_1,
                  std::placeholders::_2, id, std::stod(mat[4].str()));
  } else if (mat[2].str() ==
             JX3DPS_REGEX_EXPRESSION_EVENT_TYPE_SET_TARGET_DEAD) {
    exprEvent.second =
        std::bind(&JX3DPS::Expression::SetTargetDead, std::placeholders::_1,
                  std::placeholders::_2, id);
  }
...
const char *const JX3DPS_REGEX_EXPRESSION_EVENT_ADD_BUFF =
    "\\/add_buff id=(\\d{1,2}) name=([\u4e00-\u9fa5·0-9a-zA-Z]+)( "
    "stack_num=(\\d{1,}))?( "
    "duration=(\\d{1,})-(\\d{1,}))?";

JX3DPS::Error_t JX3DPS::Regex::ParseToAddBuff(const std::string &str,
                                              ExprEvent &exprEvent) {
  ...
  if (std::stoi(mat[1].str()) == 0) {
    ...
    id = static_cast<Id_t>(std::stoi(mat[1].str()) +
                           TARGET_PLACE_HOLDERS_DEFAULT);
  }
  ...
  exprEvent.second = std::bind(&JX3DPS::Expression::AddBuff,
                               std::placeholders::_1, std::placeholders::_2, id,
                               buffId, stackNum, durationMin, durationMax);
  return JX3DPS_SUCCESS;
}

JX3DPS::Error_t
JX3DPS::Regex::ParseToBuff3rds(const std::list<std::string> &strs,
...
const char *const JX3DPS_REGEX_EXPRESSION_EVENT_CLEAR_BUFF =
    "\\/clear_buff id=(\\d{1,2}) name=([\u4e00-\u9fa5·0-9a-zA-Z]+) "
    "stack_num=(\\d{1,})";

JX3DPS::Error_t JX3DPS::Regex::ParseToClearBuff(const std::string &str,
                                                ExprEvent &exprEvent) {
  ...
  exprEvent.second =
      std::bind(&JX3DPS::Expression::ClearBuff, std::placeholders::_1,
                std::placeholders::_2, id, buffId, stackNum);
...
JX3DPS::Error_t JX3DPS::Regex::ParseToStop(const std::string &str,
                                           ExprEvent &exprEvent) {
  exprEvent.second =
      std::bind(&JX3DPS::Expression::SetPlayerStop, std::placeholders::_1,
...
JX3DPS::Error_t JX3DPS::Regex::ParseToContinue(const std::string &str,
                                               ExprEvent &exprEvent) {
  exprEvent.second =
      std::bind(&JX3DPS::Expression::SetPlayerStop, std::placeholders::_1,
...
JX3DPS::Error_t JX3DPS::Regex::ParseToEnd(const std::string &str,
                                          ExprEvent &exprEvent) {
  exprEvent.second = std::bind(&JX3DPS::Expression::SetEnd,
                               std::placeholders::_1, std::placeholders::_2);
  return JX3DPS_SUCCESS;
}

JX3DPS::Error_t
JX3DPS::Regex::ParseToTotalFrames(const std::list<std::string> &strs,
                                  Frame_t &totalFrames) {
  for (const auto &str : strs) {
    std::regex reg(JX3DPS_REGEX_EXPRESSION_EVENT);
    std::smatch mat;
