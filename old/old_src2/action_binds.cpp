...
{
    return context.player.GetSkillCooldown(skill_id) == 0;
}

bool BuffExist(jx3id_t buff_id)
{
    return context.player.HasBuff(buff_id);
}
...
{
    return context.target->HasBuff(buff_id);
}
...
{
    return context.player.GetLastCastSkill() == skill_id;
}
...
{
    return context.player.GetBuffStackNum(buff_id) < stack_num;
}
...
{
    return context.target->GetBuffStackNum(buff_id) < stack_num;
}
...
{
    return context.player.GetBuffTime(buff_id) < tick;
}
...
{
    return context.target->GetBuffTime(buff_id) < tick;
}
...
{
    return context.player.GetSkillCooldown(skill_id) < tick;
}
...
{
    return context.player.GetSkillEnergy(skill_id) < energy;
}
...
{
    return context.targets[target_id].GetLifePercent() < ratio;
}
...
{
    return context.target->GetLifePercent() < ratio;
}
...
{
    return context.targets[target_id].GetDistance() < distance;
}
...
{
    return context.target->GetDistance() < distance;
}
...
{
    return context.player.GetManaPercent() < ratio;
}
...
{
    return context.player.GetNearbyEnemyCount(distance) < count;
}
...
{
    return context.player.GetQidianCount() < count;
}
