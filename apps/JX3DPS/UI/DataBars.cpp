 ...
 * Author: 难为水
 * -----
 * Last Modified: 2023-07-03 02:16:58
 * Modified By: 难为水
 * -----
 * HISTORY:
 * Date      	By   	Comments
...
QColor DataBars::GetColor(const double rate)
{
    if (rate > 1.0) {
        ...
        int    r = m_color0.red() + static_cast<int>((m_color2.red() - m_color0.red()) * colorRate);
        int g = m_color0.green() + static_cast<int>((m_color2.green() - m_color0.green()) * colorRate);
        int b = m_color0.blue() + static_cast<int>((m_color2.blue() - m_color0.blue()) * colorRate);
        return QColor(r, g, b);
    }
}
