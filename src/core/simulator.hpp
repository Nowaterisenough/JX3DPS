...
public:
    ...
    Simulator(const SimulatorConfig& config)
        : config_(config)
        , total_frames_(config.duration_seconds * config.frame_rate)
    {
        spdlog::info("初始化模拟器: {}秒, {}帧/秒, 共{}帧",
    ...
    SimulationStats RunSingle(ProgressCallback progress_callback = nullptr) {
        ...
        while (current_time < total_frames_) {
            // 从 SimContext 获取下一个关键帧时刻（包含宏阈值）
            tick_t next_tick = tls_sim.GetNextKeyFrameTick();
            ...
            tls_sim.Update(next_tick);

            // 当 GCD 为 0 时，尝试执行宏中的技能
            if (tls_sim.globalCooldownCurrent == 0 && macro_ast_) {
    ...
    std::vector<SimulationStats> RunMultiple(ProgressCallback progress_callback = nullptr) {
        ...
        for (int i = 0; i < config_.iterations; ++i) {
            ...
            spdlog::info("迭代 {}/{}: DPS = {:.2f}",
                         i + 1, config_.iterations, stats.average_dps);
    ...
    static SimulationStats AggregateStats(const std::vector<SimulationStats>& all_stats) {
        ...
        for (const auto& stats : all_stats) {
            ...
            if (dps > result.max_dps) result.max_dps = dps;
            if (dps < result.min_dps) result.min_dps = dps;
        }
...
private:
    ...
    void Initialize() {
        ...
        tls_sim.targets[target_->id] = target_.get();
    ...
    void TriggerExpiredEvents() {
        ...
        for (size_t i = 0; i < TickCache::MAX_SKILLS; ++i) {
            if (tls_sim.cache.skill_cooldown[i] == 0) {
                // 技能CD结束，可以触发回调或标记
                spdlog::trace("技能 {} CD结束", i);
            }
            if (tls_sim.cache.skill_prepare[i] == 0) {
                // 技能前摇结束
                spdlog::trace("技能 {} 前摇结束", i);
            }
            if (tls_sim.cache.skill_casting[i] == 0) {
                // 技能读条结束
        ...
        for (size_t u = 0; u < TickCache::MAX_UNITS; ++u) {
            for (size_t b = 0; b < TickCache::MAX_BUFFS; ++b) {
                if (tls_sim.cache.buff_duration[u][b] == 0) {
                    // BUFF持续时间结束
                    spdlog::trace("单位 {} BUFF {} 持续时间结束", u, b);
                }
                if (tls_sim.cache.buff_interval[u][b] == 0) {
                    // BUFF tick间隔到期
...
public:
     ...
     */
    void RecordDamage(jx3id_t skill_id, const std::string& skill_name,
                      long long damage, bool is_critical = false)
    {
