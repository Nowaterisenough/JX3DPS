...
void WuWoWuJian::Cast(Player &player, TargetList &targetList, Stats::ThreadStats &threadStats, Stats::SIM_MODE &simMode)
{
...
void WuWoWuJian::InitBaseParams()
{
    ...
    m_subNameVec.push_back("白虹");
    m_levelNameVec.push_back("1气点");
    m_levelNameVec.push_back("2气点");
    m_levelNameVec.push_back("3气点");
    m_levelNameVec.push_back("4气点");
    m_levelNameVec.push_back("5气点");
    m_levelNameVec.push_back("6气点");
    m_levelNameVec.push_back("7气点");
    m_levelNameVec.push_back("8气点");
    m_levelNameVec.push_back("9气点");
    m_levelNameVec.push_back("10气点");
    m_cooldown = 0;
    m_prepareFrames = -1;
    m_intervalFrames = -1;
    m_effectNum = 0;
...
{
    m_damageParamVec.push_back(Stats::DamageParam((22 + 24) / 2, 2048, 691));
    m_damageParamVec.push_back(Stats::DamageParam((44 + 49) / 2, 2048, 1405));
    m_damageParamVec.push_back(Stats::DamageParam((67 + 74) / 2, 2048, 2119));
    m_damageParamVec.push_back(Stats::DamageParam((89 + 99) / 2, 2048, 2833));
    m_damageParamVec.push_back(Stats::DamageParam((112 + 124) / 2, 2048, 3547));
    m_damageParamVec.push_back(Stats::DamageParam((134 + 149) / 2, 2048, 4261));
    m_damageParamVec.push_back(Stats::DamageParam((157 + 174) / 2, 2048, 4975));
    m_damageParamVec.push_back(Stats::DamageParam((179 + 198) / 2, 2048, 5689));
    m_damageParamVec.push_back(Stats::DamageParam((202 + 223) / 2, 2048, 6403));
    m_damageParamVec.push_back(Stats::DamageParam((224 + 248) / 2, 2048, 7117));
}
...
Stats::TableResult WuWoWuJian::GetRollResult(Player &player, Target &target)
{
    ...
    tableResult = Roll(player.GetPhysicsHitValuePercent() + m_skillHitValuePercentAdd,
                       ...
                       player.GetStrainPercent(), target.GetMissPercent(), target.GetSightPercent());
    return tableResult;
}
...
Stats::DamageStats WuWoWuJian::GetDamage(Player &player, Target &target, Stats::TableResult tableResult)
{
    ...
    int fixedDamage = static_cast<int>(m_damageParamVec
                                       [static_cast<unsigned>(m_qidian - 1)].
                                       fixedDamage * percentAdd);
    int weaponDamage = static_cast<int>((m_damageParamVec
                                        [static_cast<unsigned>(m_qidian - 1)].
                                        weaponDamageBinPercent >> 10) * player.GetWeaponAttack() * percentAdd);
    int attackDamage = static_cast<int>(m_damageParamVec
                                        [static_cast<unsigned>(m_qidian - 1)].
...
void WuWoWuJian::RecordStats(Player &player, Target &target, Stats::ThreadStats &threadStats, Stats::SIM_MODE &simMode, Stats::TableResult tableResult)
{
    ...
    case Stats::SIM_MODE::DEFAULT :
        ...
        threadStats.threadDamageStats[m_id].second[target.GetId()][m_subNameVec[0]][m_qidian - 1].
...
void WuWoWuJian::SubEffect(Player &player, TargetList &targetList, Stats::ThreadStats &threadStats, Stats::SIM_MODE &simMode, Stats::TableResult tableResult)
{
    static_cast<TaiXuJianYi *>(&player)->UpdateQidian(-10);
    if (Stats::GetRand(0, 1) < 0.25 && static_cast<TaiXuJianYi *>(&player)->m_talentKunWu && m_qidian == 10) {
    ...
    if (tableResult != Stats::TableResult::MISS) {
        if (static_cast<TaiXuJianYi *>(&player)->m_talentFengShi) {
            static_cast<FengShi *>(player.m_buffMap[BUF_FENG_SHI])->Clean(player, *targetList.front(), threadStats, simMode);
        }
        ...
        if (targetList.front()->m_buffMap[TBUF_DIE_REN]->IsExist() && static_cast<TaiXuJianYi *>(&player)->m_talentWuYu) {
            static_cast<BaHuangGuiYuan *>(player.m_skillMap[SKI_BA_HUANG_GUI_YUAN])->UpdateCooldown(8 * m_qidian);
        }
...
Stats::DamageStats WuWoWuJian::GetDamageBaiHong(Player &player, Target &target, Stats::TableResult tableResult)
{
...
void WuWoWuJian::RecordStatsBaiHong(Player &player, Target &target, Stats::ThreadStats &threadStats, Stats::SIM_MODE &simMode, Stats::TableResult tableResult)
{
    Stats::DamageStats damageStats;
    switch (static_cast<int>(simMode)) {
    case Stats::SIM_MODE::DEFAULT :
        damageStats = GetDamage(player, target, tableResult);
        threadStats.threadDamageStats[m_id].second[target.GetId()][m_subNameVec[1]][m_qidian - 1].
                second[tableResult].first++;
        threadStats.threadDamageStats[m_id].second[target.GetId()][m_subNameVec[1]][m_qidian - 1].
                second[tableResult].second += damageStats;
        threadStats.attrBonusMap[Stats::AttrType::DEFAULT].damage += damageStats.SumDamage();
...
void WuWoWuJian::SubEffectBaiHong(Player &player, Target &target, Stats::ThreadStats &threadStats, Stats::SIM_MODE &simMode, Stats::TableResult tableResult)
{
