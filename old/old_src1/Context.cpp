...
void Context::Update(tick_t tick) noexcept
{
    ...
    constexpr size_t TOTAL_ELEMENTS   = sizeof(TickCache) / sizeof(tick_t);
...
#else
    // 通用实现，不使用 SIMD 指令
    for (size_t i = 0; i < sizeof(TickCache) / sizeof(tick_t); ++i) {
...
{
    std::memset(static_cast<void *>(&context.frame_cache), 0xFF, sizeof(TickCache));
